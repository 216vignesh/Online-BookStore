...
$sql = "SELECT Book.bid as bid, Edition.Format as Format,Book.title AS Popular_In_Young_Adult, Author.name AS Author_Name, COUNT(*) AS NumberOfCopiesSold,Edition.price AS Price, RATING_VIEWS.Rating AS Rating
FROM Book
...
#$sql1= "SELECT bid, title AS Popular_In_Young_Adult, Rating FROM RATING_VIEWS where title IN (SELECT Book.title FROM Book,Info WHERE Info.genre='SciFi/Fantasy')"
