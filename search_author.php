...
if (isset($_POST['author'])) {       // SQL query to select data from database

    $_SESSION['live_search_author']=$_POST['live_search_author'];

$sql1 = "SELECT DISTINCT Book.bid as bid, Edition.Format as Format, Book.title AS Title, Author.name AS Author_Name,Edition.price AS Price, RATING_VIEWS.Rating AS Rating FROM Sales_Report INNER JOIN Edition ON Edition.isbn= Sales_Report.isbn INNER JOIN Book ON Book.bid=Edition.bid INNER JOIN Info ON Info.bid=Book.bid INNER JOIN Writes ON Book.bid=Writes.bid INNER JOIN Author ON Writes.aid=Author.aid INNER JOIN RATING_VIEWS ON RATING_VIEWS.bid=Book.bid WHERE Author.name LIKE '%{$_SESSION['live_search_author']}%'";
...
if(isset($_POST['add'])){
                ...
                $sql = "SELECT DISTINCT Book.bid as bid, Edition.Format as Format, Book.title AS Title, Author.name AS Author_Name,Edition.price AS Price, RATING_VIEWS.Rating AS Rating FROM Sales_Report INNER JOIN Edition ON Edition.isbn= Sales_Report.isbn INNER JOIN Book ON Book.bid=Edition.bid INNER JOIN Info ON Info.bid=Book.bid INNER JOIN Writes ON Book.bid=Writes.bid INNER JOIN Author ON Writes.aid=Author.aid INNER JOIN RATING_VIEWS ON RATING_VIEWS.bid=Book.bid WHERE Author.name LIKE '%{$_SESSION['live_search_author']}%'";
