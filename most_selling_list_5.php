...
$sql = "SELECT Book.bid as bid, Edition.Format as Format,Book.title AS Popular_In_Mystery, Author.name AS Author_Name, COUNT(*) AS NumberOfCopiesSold,Edition.price AS Price , RATING_VIEWS.Rating AS Rating FROM Book INNER JOIN Edition ON Edition.bid=Book.bid INNER JOIN Sales_Report ON Sales_Report.isbn=Edition.isbn INNER JOIN Info ON Info.bid=Book.bid INNER JOIN Writes ON Book.bid=Writes.bid INNER JOIN Author ON Writes.aid=Author.aid INNER JOIN RATING_VIEWS ON RATING_VIEWS.bid=Book.bid WHERE Info.genre = 'Mystery' GROUP BY Popular_In_Mystery ORDER BY COUNT(*) DESC";
...
<body>
   ...
   <div class="add-button-wrapper widget-fingerprint-product-add-button">
    ...
    <section>
        ...
        <table>
            <tr>
                <th>Popular books in Mystery</th>
                <th>Book ID</th>
                <th>Author</th>
                <th>Format</th>
                <th>Price</th>
            ...
            <tr>
                <!-- FETCHING DATA FROM EACH
                    ROW OF EVERY COLUMN -->
                <td><input type="text" name="title" value="<?php echo $rows["Popular_In_Mystery"] ?>" readonly></td>
